...
批注：两种部署方式相同参数的配置并无不同。请选择适合您生产环境的部署方式。
### 使用
...
您可点击精选服务界面，查看精选服务的详情。在点击"购买服务"后，可选择您想要购买的套餐，再填入必须的配置参数后即可购买。当跳转到支付宝并支付完后，您购买的服务实例会自动部署，当部署成功后即可使用该服务。
5. 用户自助退款
